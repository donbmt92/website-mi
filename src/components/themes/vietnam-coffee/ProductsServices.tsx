...
const ProductsServices = ({ theme, content }: ProductsServicesProps) => {
  ...
  const getTitleSize = () => getHeadingSize('large');
  ...
  const getDescriptionSize = () => getBodySize();

  // Get description weight from content
  const getDescriptionWeight = () => getWeightClasses(theme.typography?.fontWeight || 'normal');
